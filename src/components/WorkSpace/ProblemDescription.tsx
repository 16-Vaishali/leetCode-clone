...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({ problem,_solved }) => {
  ...
  const { liked, disliked, solved, setData, starred,premium } = useGetUserDataOnProblem(
    problem.id
  );
...
if(!user) {
  toast.error('You must log in to like a problem',{position:'top-left',theme:'dark'})
  return;
}
if(updating) return;
setupdating(true)
 ...
 const {userDoc,problemDoc,userRef,problemRef} = await returnData(transaction) 
  if(userDoc.exists() && problemDoc.exists()){
    if(liked){
      ...
      setCurrproblem((prev)=>prev ? {...prev,likes:prev.likes-1}:null) //updating UI
